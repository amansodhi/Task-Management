...
<body class="bg-gray-100 font-family-karla w-auto flex">
    ...
    {{-- side dashboard --}}
    <aside class="relative 600 w-54 shadow-xl">
        <div class="p-6">
            <a href="{{ route('task.index') }}"
                class="text-black text-3xl font-semibold uppercase hover:text-gray-300">Task Manager</a>
            <button
        ...
        <nav class="text-white text-base font-semibold pt-3">
            ...
            @auth
                @if(Auth::user()->admin)
                   <a href="{{ route('admin.dashboard') }}" class="flex items-center  text-black py-4 pl-6 nav-item">
                    <i class="fas fa-tachometer-alt mr-3"></i>
                    Admin Dashboard
                    </a> 
                @endif
                <a href="/user/{{ auth()->id() }}/dashboard" class="flex items-center text-black py-4 pl-6 nav-item">
                    <i class="fas fa-user mr-3"></i>
                    {{ ucwords(auth()->user()->name) }}'s Dashboard
                </a>
            @endauth
            <a href="{{ route('task.index') }}"
                class="flex items-center text-black hover:opacity-100 py-4 pl-6 nav-item">
            ...
            @auth
                <div x-data="{ isOpen: false }" class="relative w-1/2 flex justify-end">
                    ...
                    <div x-show="isOpen" class="absolute w-25 bg-white rounded-lg shadow-lg py-2 mt-6">
                        <a href="/user/{{ auth()->id() }}/dashboard" class="block px-4 py-2 account-link hover:text-black">
                            Account
                        </a>

                        <form action="/logout" method="post" class="block px-4 py-2 account-link hover:text-black">
                            @csrf
